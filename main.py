...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    layer7_upsample = tf.layers.conv2d_transpose(layer7_1x1, num_classes, 4, 2, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    layer4_upsample = tf.layers.conv2d_transpose(layer7_1x1, num_classes, 4, 2, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer4_skip = tf.add(layer7_upsample, layer4_1x1)
    layer4_skip_up = tf.layers.conv2d_transpose(layer4_skip, num_classes, 4, 2, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    layer3_skip = tf.add(layer4_skip_up, layer3_1x1)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for images,labels in get_batches_fn(batch_size):
            ...
            count = count+1
            #if count%20 == 0:
            print("Epochs-%d Loss-%f"%(epoch, loss))
...
def run():
    ...
    with tf.Session(config=config) as sess:
        ...
        train_nn(sess, epochs=20, batch_size = 8, get_batches_fn=get_batches_fn, train_op=train_op, cross_entropy_loss=cross_entropy_loss, 
                input_image=image_input, correct_label=correct_label, keep_prob=keep_prob, learning_rate=learning_rate)
